    ...
    def go route, type = :html
      ...
      body, status = if Context.new.respond_to?(:"to_#{type}")
        if route.first =~ /\d{4}/
          ...
          pages = Dir["#{Paths[:pages]}/*.rhtml"]

          pages.map! { |file| file.split('/').last.gsub(/\..*/,'') }

          if pages.include? path
            context[{}, path.to_sym]
          else
            articles = Dir["#{Paths[:articles]}/*#{path}.#{self[:ext]}"]
            # Thin::Logging.log articles.inspect
            if !articles.empty?
              found_route = [articles.first.split('/').last.gsub(/\..*/,'')]
              context[article(found_route), :article]
            else
              http 400
            end
          end
  ...
  protected
    ...
    class Context
      ...
      def initialize ctx = {}, config = {}, path = "/"
        @config, @context, @path, @env = config, ctx, path
